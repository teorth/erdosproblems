...
- Navigate to the problem page of an Erdős problem for which an OEIS sequence is marked as "possible" on the table.
...
- If no match is found, you can still report your sequence as a Github issue or on the Erdős problem web page, and we can discuss whether to submit the sequence to the OEIS (at which point we would update the table accordingly).
- If the sequence matches a seemingly unrelated sequence, this could either be a coincidence, or an unexpected connection with another problem.  It would be best to debate this on the appropriate Erdős problem web page before taking further action.
...
- The problem may ask to upper or lower bound the size of an object $A$ that is defined on some space of "size" $n$ (e.g., $A$ could range over subsets of $\{1,\dots,n\}$).  In that case, the natural sequence to consider is the maximal or minimal size of such an object, as a function of $n$.
...
If you have further questions about how to extract an integer sequence from a given Erdős porblem, you can raise a Github issue or make a comment on the appropriate [erdosproblems.com](https://www.erdosproblems.com) forum page.
